...
Mesh::Mesh(VulkanEngine& renderer, VkDescriptorSetLayout desc, Texture textureManager, const MeshData& meshData) : BaseMesh(renderer, meshData)
{
 ...
 //   LoadTextures(renderer, textureManager, meshData.TextureList);
    ...
    CreateDescriptorSets(renderer, desc, textureManager);
...
Mesh::Mesh(VulkanEngine& renderer, VkDescriptorSetLayout desc, Texture textureManager, const std::vector<Vertex>& vertexdata, const std::vector<uint16_t>& indicesdata, MeshTextures textures) : BaseMesh(renderer, vertexdata, indicesdata)
{
    CustomBuffer EmptyBuffer;
    EmptyBuffer.ByteSize = sizeof(Empty);
...
Mesh::Mesh(VulkanEngine& renderer, VkDescriptorSetLayout desc, Texture textureManager, const std::vector<Vertex>& vertexdata, const std::vector<uint16_t>& indicesdata, MeshTextures textures, CustomBuffer customBuffer) : BaseMesh(renderer, vertexdata, indicesdata)
{
...
void Mesh::ScreenResizeUpdate(VulkanEngine& renderer, VkDescriptorSetLayout desc, Texture textureManager)
{
    CreateDescriptorPool(renderer);
    CreateDescriptorSets(renderer, desc, textureManager);
}
